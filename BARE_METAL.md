...
`git clone git@github.com:Mintplex-Labs/mind-law.git`

2. `cd mind-law` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
cd $HOME/mind-law/frontend && yarn && yarn build && cd $HOME/mind-law
...
cd $HOME/mind-law/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/mind-law/server && yarn
cd $HOME/mind-law/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/mind-law/server && npx prisma generate

echo "Booting up services."
truncate -s 0 /logs/server.log # Or any other log file location.
truncate -s 0 /logs/collector.log

cd $HOME/mind-law/server
(NODE_ENV=production node index.js) &> /logs/server.log &

cd $HOME/mind-law/collector
